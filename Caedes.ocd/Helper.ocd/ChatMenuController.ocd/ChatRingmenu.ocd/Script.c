...
static const GUI_ChatRingmenu_Radius = 225;

static const GUI_ChatRingmenu_Ring = 70;
...
global func CreateChatRingMenu(id symbol, object commander)
{
	if(!(this->GetOCF() & OCF_CrewMember)) return nil;
	if(!(this->~HasMenuControl())) return nil;
	var menu = CreateObject(GUI_ChatRingMenu,0,0,this->GetOwner());
	
	// minimum padding:
	var paddingy = GUI_ChatRingmenu_Radius + 175;
	var paddingx = GUI_ChatRingmenu_Radius + 50;
...
public func AddItem(new_item, int amount, extra)
{
	...
	menu_icons[index] = CreateObject(GUI_ChatRingMenu_Icon,0,0,menu_object->GetOwner());
...
func OnMouseClick(int dx, int dy, bool alt)
{
	...
	if(distance < GUI_ChatRingmenu_Ring)
		Close();
...
func Show()
{
	...
	for(var i=0; i<item_count; i++)
	{
		if(menu_icons[i])
		{
			...
			if(item_count == 1)
				menu_icons[i]->SetPosition(x,y+GUI_ChatRingmenu_Radius);
			else
			{
				var add = 0, dadd = 0;
				if(angle > 180) 
				{
					add = 180; 
					var s = menu_icons[i]->GetSymbol();
					dadd = (5 * s->GetDefCoreVal("Offset", "DefCore", 0)) / 2;
				}
				
				menu_icons[i]->SetPosition(x+Sin(angle,GUI_ChatRingmenu_Radius + dadd),y-Cos(angle,GUI_ChatRingmenu_Radius + dadd));
				
				menu_icons[i]->SetAngle(360 - angle + 90 + add);
				menu_icons[i]->SetSize(2000);
				menu_icons[i]->SetBG(false);
			}
...
public func UpdateCursor(int dx, int dy)
{
	if(shown)
	{
		...
		var distance = Sqrt(dx*dx+dy*dy);
		var outside = distance < GUI_ChatRingmenu_Ring;
		if(outside)
		{
		...
		for(var i=0; i< item_count; i++)
		{
			if(menu_icons[i])
			{
					...
					if(i == item)
					{
						...
						menu_icons[i]->Hover(true);
					} else menu_icons[i]->Hover(false);
			}
		}
	}
}
